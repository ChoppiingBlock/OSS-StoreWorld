 ...
 */
    ...
    public void sendFileStreamByPath(String path, int index, int offset, HttpServletResponse response) throws IOException {
        //index代表的是分片编号，从0开始

        logger.info("文件发送{}，{}", path, index);
        ...
        int fileLength = 0;
        //计算偏移，计算文件长度
        if(index < FileMaxChunk(file) - 1) {
            //分块为非最后一块
            fileLength = SystemConstants.shadeSize;
        } else if(index == FileMaxChunk(file) - 1){
            fileLength = (int) (file.length() - (FileMaxChunk(file) - 1) * SystemConstants.shadeSize - offset);
        } else {
        ...
        inputStream.skip((long) SystemConstants.shadeSize * index + offset);
